...
class StoryItem(Base):
    ...
    start_time_ms = Column(
        Integer, nullable=False, default=0
    )  # Milliseconds from story start
    track = Column(Integer, nullable=False, default=0)  # Track number (0 = main track)
    trim_start_ms = Column(
        Integer, nullable=False, default=0
    )  # Milliseconds trimmed from start
    trim_end_ms = Column(
        Integer, nullable=False, default=0
    )  # Milliseconds trimmed from end
    created_at = Column(DateTime, default=datetime.utcnow)
...
def init_db():
    ...
    try:
        default_channel = (
            db.query(AudioChannel).filter(AudioChannel.is_default == True).first()
        )
        if not default_channel:
            default_channel = AudioChannel(
                id=str(uuid.uuid4()), name="Default", is_default=True
            )
            ...
            for profile in profiles:
                mapping = ProfileChannelMapping(
                    profile_id=profile.id, channel_id=default_channel.id
                )
                db.add(mapping)
...
def _run_migrations(engine):
        ...
        with engine.connect() as conn:
            ...
            if not has_start_time:
                ...
                for row in rows:
                    ...
                    conn.execute(
                        text(
                            "UPDATE story_items SET start_time_ms = :time WHERE id = :id"
                        ),
            ...
            conn.commit()
            print(
                "Migrated story_items table to use start_time_ms (removed position column)"
            )
    ...
    if "track" not in columns:
        ...
        with engine.connect() as conn:
            conn.execute(
                text(
                    "ALTER TABLE story_items ADD COLUMN track INTEGER NOT NULL DEFAULT 0"
                )
            )
            conn.commit()
    ...
    if "trim_start_ms" not in columns:
        ...
        with engine.connect() as conn:
            conn.execute(
                text(
                    "ALTER TABLE story_items ADD COLUMN trim_start_ms INTEGER NOT NULL DEFAULT 0"
                )
            )
            conn.commit()
    ...
    if "trim_end_ms" not in columns:
        ...
        with engine.connect() as conn:
            conn.execute(
                text(
                    "ALTER TABLE story_items ADD COLUMN trim_end_ms INTEGER NOT NULL DEFAULT 0"
                )
            )
            conn.commit()
    ...
    if "profiles" in inspector.get_table_names():
        ...
        if "avatar_path" not in columns:
            ...
            with engine.connect() as conn:
                conn.execute(
                    text("ALTER TABLE profiles ADD COLUMN avatar_path VARCHAR")
                )
                conn.commit()
