...
class MLXTTSBackend:
    ...
    def _is_model_cached(self, model_size: str) -> bool:
        ...
        try:
            ...
            if blobs_dir.exists() and any(blobs_dir.glob("*.incomplete")):
                print(
                    f"[_is_model_cached] Found .incomplete files for {model_size}, treating as not cached"
                )
                return False
            ...
            if snapshots_dir.exists():
                has_weights = (
                    any(snapshots_dir.rglob("*.safetensors"))
                    or any(snapshots_dir.rglob("*.bin"))
                    or any(snapshots_dir.rglob("*.npz"))
                )
                if not has_weights:
                    print(
                        f"[_is_model_cached] No model weights found for {model_size}, treating as not cached"
                    )
                    return False
    ...
    def _load_model_sync(self, model_size: str):
        ...
        try:
            ...
            print(
                f"Error: mlx_audio package not found. Install with: pip install mlx-audio"
            )
            progress_manager = get_progress_manager()
            task_manager = get_task_manager()
            model_name = f"qwen-tts-{model_size}"
        ...
        if use_cache:
            ...
            if cached_prompt is not None:
                ...
                if isinstance(cached_prompt, dict):
                    ...
                    cached_audio_path = cached_prompt.get(
                        "ref_audio"
                    ) or cached_prompt.get("ref_audio_path")
                    if cached_audio_path and Path(cached_audio_path).exists():
                        return cached_prompt, True
                    else:
                        # Cached file no longer exists, invalidate cache
                        print(
                            f"Cached audio file not found: {cached_audio_path}, regenerating prompt"
                        )
    ...
    async def generate(
        ...
        Args:
            ...
            language: Language code (en, zh, tl, ilo, etc.)
        ...
        def _generate_sync():
            ...
            ref_audio = voice_prompt.get("ref_audio") or voice_prompt.get(
                "ref_audio_path"
            )
            ...
            if ref_audio:
                if not Path(ref_audio).exists():
                    raise ValueError(
                        f"Voice cloning audio file not found: {ref_audio}. "
                        f"Please re-upload your voice sample or check the profile."
                    )
                ...
                print(f"  Voice cloning: DISABLED (no reference audio)")

            # Check model's generate method signature
            import inspect

            sig = inspect.signature(self.model.generate)
            supports_ref_audio = "ref_audio" in sig.parameters
            supports_language = "language" in sig.parameters

            print(f"  Model supports ref_audio: {supports_ref_audio}")
            print(f"  Model supports language: {supports_language}")

            # Build generation kwargs
            gen_kwargs = {"text": text}

            if ref_audio and supports_ref_audio:
            ...
            try:
                if ref_audio and not supports_ref_audio:
                    # Model doesn't support voice cloning
                    raise ValueError(
                        f"The current TTS model does not support voice cloning. "
                        f"Voice samples will be ignored. To use voice cloning, "
                        f"ensure you're using a model that supports ref_audio parameter."
                    )
                ...
                # Re-raise ValueError (our error messages)
                raise
            except TypeError as e:
                # Parameter mismatch - model doesn't support some parameters
                if "language" in str(e) and supports_language:
            ...
            if audio_chunks:
                audio = np.concatenate(
                    [np.asarray(chunk, dtype=np.float32) for chunk in audio_chunks]
                )
            else:
...
class MLXSTTBackend:
    ...
    def _is_model_cached(self, model_size: str) -> bool:
        ...
        try:
            ...
            if blobs_dir.exists() and any(blobs_dir.glob("*.incomplete")):
                print(
                    f"[_is_model_cached] Found .incomplete files for whisper-{model_size}, treating as not cached"
                )
                return False
            ...
            if snapshots_dir.exists():
                ...
                if not has_weights:
                    print(
                        f"[_is_model_cached] No model weights found for whisper-{model_size}, treating as not cached"
                    )
                    return False
            ...
            print(
                f"[_is_model_cached] Error checking cache for whisper-{model_size}: {e}"
            )
            return False
